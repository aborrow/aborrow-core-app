(function () {
  ...
  dd.directive('rsmdatedropdowns', ['rsmdateutils', function (rsmdateutils) {
    return {
      ...
      '     <select name="dateFields.day" data-ng-model="dateFields.day" placeholder="Day" class="input-info small" ng-options="day for day in days" ng-change="checkDate()" ng-disabled="disableFields"><option value="">วัน</option></select>' +
      '  </div>' +
      '  <div class="form-group col-xs-5">' +
      '    <select name="dateFields.month" data-ng-model="dateFields.month" placeholder="Month" class="input-info small" ng-options="month.value as month.name for month in months" value="{{ dateField.month }}" ng-change="checkDate()" ng-disabled="disableFields"><option value="">เดือน</option></select>' +
      '  </div>' +
      '  <div class="form-group col-xs-4">' +
      '    <select ng-show="!yearText" name="dateFields.year" data-ng-model="dateFields.year" placeholder="Year" class="input-info small" ng-options="year for year in years" ng-change="checkDate()" ng-disabled="disableFields"><option value="">ปี</option></select>' +
      '    <input ng-show="yearText" type="text" name="dateFields.year" data-ng-model="dateFields.year" placeholder="Year" class="input-info" ng-disabled="disableFields">' +
      '  </div>' +
      '</div>',
      link: function (scope, element, attrs, ctrl) {
        var currentYear = (parseInt(attrs.startingYear, 10) || new Date().getFullYear())+543,
